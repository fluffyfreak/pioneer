...
TriangleGraphic::TriangleGraphic(Renderer *renderer, Graphics::RenderState *rs, VertexArray *v, RefCountedPtr<Graphics::Material> m, Graphics::PrimitiveType pt)
: Renderable(renderer, rs)
, m_vertexArray(v)
, m_material(m)
, m_primitiveType(pt)
{
...
void TriangleGraphic::Draw(Graphics::RenderState *rs)
{
	...
	m_renderer->DrawTriangles(m_vertexArray.get(), (m_renderState!=nullptr) ? m_renderState : rs, m_material.Get(), m_primitiveType);
}

LaserGraphic::LaserGraphic(Renderer *r, Graphics::RenderState *rs)
: Renderable(r, rs)
, m_color(Color::WHITE)
, m_sideIntensity(0.f)
, m_glowIntensity(0.f)
...
void LaserGraphic::Draw(Graphics::RenderState *rs)
{
	...
	if (m_sideIntensity > 0.01f) {
		...
		GetRenderer()->DrawTriangles(Projectile::GetSideVerts(), (m_renderState!=nullptr) ? m_renderState : rs, Projectile::GetSideMat());
	}
	if (m_glowIntensity > 0.01f) {
		Projectile::GetGlowMat()->diffuse = m_color * m_glowIntensity;
		GetRenderer()->DrawTriangles(Projectile::GetGlowVerts(), (m_renderState!=nullptr) ? m_renderState : rs, Projectile::GetGlowMat());
	}
}
