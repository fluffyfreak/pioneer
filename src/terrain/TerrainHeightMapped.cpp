...
//	}
...

	{
		...
		double mountains = h*h*0.001*octavenoise(GetFracDef(3), 0.5*octavenoise(GetFracDef(5), 0.45, p),
			p)*ridged_octavenoise(GetFracDef(4), 0.475*octavenoise(GetFracDef(6), 0.4, p), p);
		v += mountains;
		...
		//v += Clamp(h, 0.0, 0.5)*octavenoise(GetFracDef(2), 0.5, p);

		//low altitude detail/dunes
		//v += h*0.000003*ridged_octavenoise(GetFracDef(2), Clamp(1.0-h*0.002, 0.0, 0.5), p);
		if (v < 10.0){
			v += 2.0*v*dunes_octavenoise(GetFracDef(6), 0.5, p)
				*octavenoise(GetFracDef(6), 0.5, p);
		} else if (v <50.0){
			v += 20.0*dunes_octavenoise(GetFracDef(6), 0.5, p)
				*octavenoise(GetFracDef(6), 0.5, p);
		} else {
			v += (50.0/v)*(50.0/v)*(50.0/v)*(50.0/v)*(50.0/v)
				*20.0*dunes_octavenoise(GetFracDef(6), 0.5, p)
				*octavenoise(GetFracDef(6), 0.5, p);
		}
		if (v<40.0) {
			//v = v;
		...
		//ridges and bumps
		//v += h*0.1*ridged_octavenoise(GetFracDef(6), Clamp(h*0.0002, 0.3, 0.5), p)
		//	* Clamp(h*0.0002, 0.1, 0.5);
		v += h*0.2*voronoiscam_octavenoise(GetFracDef(5), Clamp(1.0-(h*0.0002), 0.0, 0.6), p)
