...
bool WindowSDL::CreateWindowAndContext(const char *name, int w, int h, bool fullscreen, bool hidden, int samples, int depth_bits, bool useOVR) {
	Uint32 winFlags = SDL_WINDOW_OPENGL | (hidden ? SDL_WINDOW_HIDDEN : SDL_WINDOW_SHOWN);
	if (!hidden && fullscreen && !useOVR) winFlags |= SDL_WINDOW_FULLSCREEN;
...
WindowSDL::WindowSDL(const Graphics::Settings &vs, const std::string &name)
{
	...
	// attempt sequence is:
	// 1- requested mode
	ok = CreateWindowAndContext(name.c_str(), vs.width, vs.height, vs.fullscreen, vs.hidden, vs.useOVR, vs.requestedSamples, 24);
	...
	if (!ok && vs.requestedSamples) {
		...
		ok = CreateWindowAndContext(name.c_str(), vs.width, vs.height, vs.fullscreen, vs.hidden, vs.useOVR, 0, 24);
	}

	// 3- requested mode with 16 bit depth buffer
	if (!ok) {
		Output("Failed to set video mode. (%s). Re-trying with 16-bit depth buffer\n", SDL_GetError());
		ok = CreateWindowAndContext(name.c_str(), vs.width, vs.height, vs.fullscreen, vs.hidden, vs.useOVR, vs.requestedSamples, 16);
	}
	...
	if (!ok && vs.requestedSamples) {
		...
		ok = CreateWindowAndContext(name.c_str(), vs.width, vs.height, vs.fullscreen, vs.hidden, vs.useOVR, 0, 16);
	}
