...
#ifndef _OGL_GEORINGMATERIAL_H
#define _OGL_GEORINGMATERIAL_H
/*
...
namespace Graphics {
	namespace OGL {
		class GeoRingProgram : public Program {
		public:
			GeoRingProgram(const std::string &filename, const std::string &defines);

			Uniform atmosColor;
			Uniform geoRingAtmosFogDensity;
			Uniform geoRingAtmosInvScaleHeight;
			Uniform geoRingAtmosTopRad; // in planet radii
			Uniform geoRingCenter;
			Uniform geoRingRadius; // (planet radius)
			Uniform geoRingInvRadius; // 1.0 / (planet radius)
		...
		class GeoRingSurfaceMaterial : public Material {
		public:
			GeoRingSurfaceMaterial();
