...
WaterProgram::WaterProgram(const std::string &filename, const std::string &defines)
{
	m_name = filename;
	m_defines = defines;
...
void WaterProgram::InitUniforms()
{
...
Program *WaterMaterial::CreateProgram(const MaterialDescriptor &desc)
{
	assert(desc.effect == EFFECT_WATER);
	...
	if (desc.quality & HAS_ECLIPSES)
		ss << "#define ECLIPSE\n";
	return new Graphics::OGL::WaterProgram("geosphere_terrain", ss.str());
}

void WaterMaterial::Apply()
{
	SetGSUniforms();
}

void WaterMaterial::SetGSUniforms()
{
	OGL::Material::Apply();

	WaterProgram *p = static_cast<WaterProgram*>(m_program);
