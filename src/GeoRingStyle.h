#ifndef __GEORINGSTYLE_H__
#define __GEORINGSTYLE_H__
#include "libs.h"
#include "StarSystem.h"

#define GEOSPHERESTYLE_MAX_OCTAVES 24

struct ringfracdef_t {
	double amplitude;
	double frequency;
	double lacunarity;
	int octaves;
...
class GeoRingStyle {
	...
	GeoRingStyle(const SBody *body);
	~GeoRingStyle() {
		if (m_heightMap) delete [] m_heightMap;
		m_heightMap = 0;
	}
	GeoRingStyle(): m_terrainType(TERRAIN_NONE), m_colorType(COLOR_NONE) {
		m_heightMap = 0;
	}
	...
	void SetFracDef(struct ringfracdef_t *def, double featureHeightMeters, double featureWidthMeters, MTRand &rand, double smallestOctaveMeters = 20.0);
	...
	ringfracdef_t m_fracdef[10];
};

#endif /* __GEORINGSTYLE_H__ */
