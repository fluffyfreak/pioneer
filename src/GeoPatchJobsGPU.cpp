...
void BasePatchJobGPU::GenerateMesh(double *heights, vector3f *normals, Color3ub *colors, 
...
// Overloaded PureJob class to handle generating the mesh for each patch
// ********************************************************************************
void SinglePatchJobGPU::OnFinish()  // runs in primary thread of the context
{
	GeoSphere::OnAddSingleResultGPU(mData->sysPath, mpResults);
	mpResults = nullptr;
	BasePatchJobGPU::OnFinish();
}

void SinglePatchJobGPU::OnRun()    // RUNS IN ANOTHER THREAD!! MUST BE THREAD SAFE!
{
	BasePatchJobGPU::OnRun();

	const SSingleRequestGPU &srd = *mData;
	...
	SSingleResultGPU *sr = new SSingleResultGPU(srd.patchID.GetPatchFaceIdx(), srd.depth);
...
SinglePatchJobGPU::~SinglePatchJobGPU()
{
	if(mpResults) {
		mpResults->OnCancel();
		delete mpResults;
...
// Overloaded PureJob class to handle generating the mesh for each patch
// ********************************************************************************
void QuadPatchJobGPU::OnFinish()  // runs in primary thread of the context
{
	GeoSphere::OnAddQuadResultGPU( mData->sysPath, mpResults );
	mpResults = nullptr;
	BasePatchJobGPU::OnFinish();
}

void QuadPatchJobGPU::OnRun()    // RUNS IN ANOTHER THREAD!! MUST BE THREAD SAFE!
{
	BasePatchJobGPU::OnRun();

	const SQuadRequestGPU &srd = *mData;
	...
	SQuadResultGPU *sr = new SQuadResultGPU(srd.patchID.GetPatchFaceIdx(), srd.depth);
	for (int i=0; i<4; i++)
	{
		// fill out the data
...
QuadPatchJobGPU::~QuadPatchJobGPU()
{
	if(mpResults) {
		mpResults->OnCancel();
		delete mpResults;
