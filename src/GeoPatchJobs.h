...
public:
	struct SSplitResultData {
		...
		SSplitResultData(double *heights_, vector3f *n_, vector2f *uvs_, const vector3d &v0_, const vector3d &v1_, const vector3d &v2_, const vector3d &v3_, const GeoPatchID &patchID_) :
			heights(heights_), normals(n_), UVs(uvs_), v0(v0_), v1(v1_), v2(v2_), v3(v3_), patchID(patchID_)
		{}
...
public:
	...
	void addResult(const int kidIdx, double *h_, vector3f *n_, vector2f *uvs_, const vector3d &v0_, const vector3d &v1_, const vector3d &v2_, const vector3d &v3_, const GeoPatchID &patchID_)
	{
		assert(kidIdx>=0 && kidIdx<NUM_RESULT_DATA);
		mData[kidIdx] = (SSplitResultData(h_, n_, uvs_, v0_, v1_, v2_, v3_, patchID_));
	}
...
public:
	...
	void addResult(double *h_, vector3f *n_, vector2f *uvs_, const vector3d &v0_, const vector3d &v1_, const vector3d &v2_, const vector3d &v3_, const GeoPatchID &patchID_)
	{
		mData = (SSplitResultData(h_, n_, uvs_, v0_, v1_, v2_, v3_, patchID_));
	}
...
private:
	...
	void GenerateSubPatchData(double *heights, vector3f *normals, vector2f *UVs, double *borderHeights, vector3d *borderVertexs,
		const vector3d &v0, const vector3d &v1, const vector3d &v2, const vector3d &v3,
