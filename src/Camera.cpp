...
void CameraContext::ApplyDrawTransforms(Graphics::Renderer *r, const ViewEye eye)
{
	if(OculusRiftInterface::HasHMD()) {
		...
		r->SetPerspectiveProjection(YFOV, GetZFar(), persp);
	} else {
		r->SetPerspectiveProjection(
			GetFovAng(), 
			GetWidth()/GetHeight(), 
			GetZNear(), 
			GetZFar());
...
void Camera::Draw(const Body *excludeBody, ShipCockpit* cockpit, const ViewEye eye)
{
	PROFILE_SCOPED()
...

	{
		...
		m_renderer->SetLights(m_rendererLights.size(), &m_rendererLights[0]);
	}
