...
const float* GetColorPtr(const Color4f& col) {
	return &col.r;
}

const float* GetVectorPtr(const vector3f& vec) {
	return &vec.x;
}
...
void VolumetricClouds::GrowCloud(VolumetricCloud* Cloud, int level, float radius, vector3f Position)
{
	...
	for (int i = 0; i < Num; i++)
	{
		...
		Cloud->BoundingBox1 = vector3f(-1000, -1000, -1000);
		Cloud->BoundingBox2 = vector3f(1000, 1000, 1000);
	...
	Cloud->TexCoordBuffer = new vector2f[Num * 4];
...
int VolumetricClouds::Create(Graphics::Renderer *renderer, const int Num, const float PlaneSize, const float PlaneHeight)
{
	...
	for (int i = 0; i < Num; i++)
	{
		...
		Cloud.Radius = 15.0f;
		Cloud.LastLight = vector3f(0, 0, 0);
		Cloud.LastCamera = vector3f(0, 0, 0);
		Cloud.ImpostorTex = 0;
...
void util_compat_gluLookAt(GLfloat eyeX, GLfloat eyeY, GLfloat eyeZ, GLfloat lookAtX, GLfloat lookAtY, GLfloat lookAtZ, GLfloat upX, GLfloat upY, GLfloat upZ) {
    vector3f x, y, z;
    z = vector3f(eyeX-lookAtX, eyeY-lookAtY, eyeZ-lookAtZ).Normalized();
...
int glhProjectf(float objx, float objy, float objz, double *modelview, double *projection, int *viewport, double *wcx, double *wcy, double *wcz)
  {
...
void VolumetricClouds::MakeCloudImpostor(Graphics::Renderer *renderer, VolumetricCloud* Cloud, vector3f Sun, vector3f Camera)
{
	...
	glPushMatrix();
	glLoadIdentity();
	util_compat_gluLookAt(Camera.x, Camera.y, Camera.z, Cloud->Center.x, Cloud->Center.y, Cloud->Center.z, 0, 1, 0);
	...
	vector3f* vp; vector2f* tp; Color4f* cp;
	vp = Cloud->VertexBuffer;
	tp = Cloud->TexCoordBuffer;
	cp = Cloud->ColorBuffer;
	
	vector2f v1(1.0f, 0.0f), v2(0.0f, 0.0f), v3(0.0f, 1.0f), v4(1.0f, 1.0f);
...
//	puffs are now in world space
	...
	glPushMatrix();
	glLoadIdentity();		
	util_compat_gluLookAt(Sun.x, Sun.y, Sun.z, Cloud->Center.x, Cloud->Center.y, Cloud->Center.z, 0, 1, 0);
	...
	Color4f LightColor(1.0f, 1.0f, 1.0f, 1.0f), ParticleColor;
	float factor = 1.0f;
	int ReadX, ReadY;
	
	vector3f Light = Sun.Normalized();

	factor = SolidAngle / (4 * M_PI);
	
	for (i = 0; i < Cloud->Puffs.size(); i++)
	{		
		//find the particle's projected coordinates
		double CenterX, CenterY, CenterZ;
		glhProjectf(Cloud->Puffs[i].Position.x, 
		...
		Cloud->Puffs[i].Color.Limit();

		//the phase function (it's always 1.5f when we're looking from the sun)
		ParticleColor = ParticleColor * 1.5f;
		ParticleColor.Limit();
				
		glColor4fv(GetColorPtr(ParticleColor));
		
		glBegin(GL_QUADS);
				
		glTexCoord2f(1.0f, 0.0f);
		glVertex3fv(GetVectorPtr(Cloud->Puffs[i].Position + (vx + vy) * -Cloud->Puffs[i].Size));		
		glTexCoord2f(0.0f, 0.0f); 		
		glVertex3fv(GetVectorPtr(Cloud->Puffs[i].Position + (vx - vy) *  Cloud->Puffs[i].Size));		
		glTexCoord2f(0.0f, 1.0f); 		
		glVertex3fv(GetVectorPtr(Cloud->Puffs[i].Position + (vx + vy) *  Cloud->Puffs[i].Size)); 		
		glTexCoord2f(1.0f, 1.0f); 		
		glVertex3fv(GetVectorPtr(Cloud->Puffs[i].Position + (vy - vx) *  Cloud->Puffs[i].Size));
...
void VolumetricClouds::RenderCloudImpostor(Graphics::Renderer *renderer, VolumetricCloud* Cloud, float alpha)
{
	...
	glTexCoord2f(0.0f, 0.0f); 			
	glVertex3fv(GetVectorPtr(Cloud->Center + (Cloud->vx + Cloud->vy) * -Cloud->Radius));
	...
	glVertex3fv(GetVectorPtr(Cloud->Center + (Cloud->vy - Cloud->vx) *  Cloud->Radius));
