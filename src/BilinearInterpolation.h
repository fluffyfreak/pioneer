...
T BilinearSample(const int imgDim, const int imgX, const int imgY, const int gridSize, const T *grid2d)
{
	...
	const float gx = imgX / float(imgDim) * gridSize; // be careful to interpolate boundaries 
	const float gy = imgY / float(imgDim) * gridSize; // be careful to interpolate boundaries 
	const int gxi = int(gx); 
	const int gyi = int(gy); 
	const T &c00 = grid2d[gyi * (gridSize + 1) + gxi]; 
	const T &c10 = grid2d[gyi * (gridSize + 1) + (gxi + 1)]; 
	const T &c01 = grid2d[(gyi + 1) * (gridSize + 1) + gxi]; 
	const T &c11 = grid2d[(gyi + 1) * (gridSize + 1) + (gxi + 1)]; 
